...
OpenMock places a screenshot or screen recording on a 3D device, lets you frame it with a camera, add depth of field and other effects, animate everything on a keyframe timeline, and export an image or a video. It all runs client side. No account, no server, no watermark, no locked features.
